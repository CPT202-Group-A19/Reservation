...
=======
        ...
        /* Determine whether the number reaches the upper limit */
        if (count > maxCount) {
            throw new InformationCountLimitException("创建信息数量已经达到上限(" + maxCount + ")！");
        }

        /* Completion data: the names of d, m, g */
        String departmentCode = dmgService.getCodeByName(information.getDepartmentName());
        int dCode=Integer.parseInt(departmentCode);
        for(int i=100;i<=5000;i+=100){
            String tmpCode=""+(dCode+i);
            if(dmgService.getNameByCode(tmpCode).equals(information.getMajorName())){
                System.out.println(dmgService.getNameByCode(tmpCode));
                dCode+=i;
                break;
            }
        }
        //String majorCode = dmgService.getCodeByName(information.getMajorName());
        String majorCode=""+dCode;
        int gCode=Integer.parseInt(majorCode);
...
=======
        /* Insert  information data and get the number of affected rows returned */
        Integer rows = informationMapper.insert(information);
...
=======
        ...
        /* Determine whether the number of affected rows is not 1 */
        if (rows != 1) {
            throw new UpdateException("设置默认信息数据时出现未知错误[2]");
        }
>>>>>>> 01657420de04e445b55df4d61bcef89c175406e8
    }
...
=======
      ...
      public void delete(Integer rid, Integer uid, String username) {
          ...
          /* Determine if the query result is null */
          if (result == null) {
              throw new InformationNotFoundException("尝试访问的用户信息不存在");
          }
          ...
          Integer rows1 = informationMapper.deleteByRid(rid);
          if (rows1 != 1) {
              throw new DeleteException("删除用户信息时出现未知错误，请联系系统管理员");
          }
          ...
          /* Determine whether the number of affected rows is not 1 */
          if (rows2 != 1) {
              throw new UpdateException("更新用户信息时出现未知错误，请联系系统管理员");
          }
      }
>>>>>>> 01657420de04e445b55df4d61bcef89c175406e8
...
=======
      ...
      //same stru above, modify reservation information
    public void modifyInformation(Integer uid, String username, Information information){
        ...
        information.setModifiedUser(username);
        information.setModifiedTime(now);
        Integer rows=informationMapper.updateDetailsByRid(information);
        if (rows != 1) {
            throw new UpdateException("设置信息数据时出现未知错误");
        }
    }
}
>>>>>>> 01657420de04e445b55df4d61bcef89c175406e8
